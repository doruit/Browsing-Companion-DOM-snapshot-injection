...
**Tech Stack**: React 18 + TypeScript + Vite | Node.js + Express | Python + FastAPI | Azure OpenAI + Cosmos DB + Blob Storage | Azure Bicep IaC
...
**💰 Estimated Cost**: Cosmos DB free tier + OpenAI pay-as-you-go (~\$0.01-0.03/chat) + minimal storage
...
cd services/ai-service && source venv/bin/activate && python -m uvicorn main:app --reload --port 8000
...
|---------|---------------|
| AI Service | \`AZURE_OPENAI_ENDPOINT\`, \`AZURE_OPENAI_API_KEY\`, \`COSMOS_CONNECTION_STRING\` |
...
This repository is **open source** and demonstrates **Method 1** (Client-Side DOM Snapshot). For production implementations of Methods 3-6, I'm available for commercial engagements.
